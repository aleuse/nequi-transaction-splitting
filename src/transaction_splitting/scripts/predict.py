...
from src.transaction_splitting.models import FractionmentDetectionEnsemble, TransactionFractionmentPredictor
...
def parse_args():
    ...
    parser.add_argument(
        "--dbscan-model-path", 
        type=str,
        help="Path to DBSCAN model"
    )
    
    parser.add_argument(
        "--scaler-path", 
        type=str,
        help="Path to scaler model"
    )
    ...
    parser.add_argument(
        "--alerts-only", 
        action="store_true",
        help="Return only records with alerts"
    )
...
def main():
    ...
    try:
        ...
        if args.verbose and num_alerts > 0:
            avg_risk_score = predictions_df.filter(pl.col("is_fractionment_alert"))["combined_risk_score"].mean()
