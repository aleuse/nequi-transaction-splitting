...
def parse_args():
    ...
    parser.add_argument(
        "--input-file", type=str, help="Path to input data file (parquet format)"
    )

    parser.add_argument(
        "--output-file", type=str, help="Path to save processed data (parquet format)"
    )
    ...
    parser.add_argument(
        "--output-features", type=str, help="Path to save feature matrix (numpy format)"
    )

    parser.add_argument("--save-scaler", type=str, help="Path to save fitted scaler")
...
def main():
    ...
    try:
        ...
        if args.output_features:
            ...
            np.save(
                features_path.with_name(features_path.stem + "_scaled.npy"), X_scaled
            )
            print(
                f"Features saved to: {features_path} and {features_path.with_name(features_path.stem + '_scaled.npy')}"
            )
        ...
        if args.verbose:
            ...
            print(
                f"  Scaled features range: [{X_scaled.min():.4f}, {X_scaled.max():.4f}]"
            )
