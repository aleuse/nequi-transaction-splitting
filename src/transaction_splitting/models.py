...
class FractionmentDetectionEnsemble:
    ...
    def predict_scores(self, X: np.ndarray) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
    ...
    def predict_alerts(
        self, 
        X: np.ndarray, 
        threshold_percentile: float = ALERT_THRESHOLD_PERCENTILE
    ...
    def _calculate_training_metrics(self, X: np.ndarray, dbscan_labels: np.ndarray) -> Dict[str, float]:
        """
...
class TransactionFractionmentPredictor:
    ...
    def predict_fractionment(
        self, 
        candidate_groups_df: pl.DataFrame,
        ...
        # Get predictions
        if_scores, dbscan_noise, combined_scores = self.ensemble.predict_scores(X_scaled)
