...
class TransactionRecord(BaseModel):
    ...
    transaction_date: Union[datetime, str] = Field(
        ...
        if isinstance(v, str):
            try:
                ...
                try:
                    ...
                    raise ValueError(
                        f"Invalid date format: {v}. Use ISO format or 'YYYY-MM-DD HH:MM:SS'"
                    )
        return v
...
class TransactionBatch(BaseModel):
    ...
    transactions: List[TransactionRecord] = Field(
        ..., min_items=1, description="List of transactions"
    )
...
class PredictionResult(BaseModel):
    ...
    is_fractionment_alert: bool = Field(
        ..., description="Whether this is flagged as fractionment"
    )
    ...
    isolation_forest_score: Optional[float] = Field(
        None, description="Isolation Forest anomaly score"
    )
...
class PredictionConfig(BaseModel):
    ...
    threshold_percentile: float = Field(
        default=95, ge=50, le=99.9, description="Alert threshold percentile"
    )
    include_scores: bool = Field(
        default=True, description="Include individual model scores"
    )
