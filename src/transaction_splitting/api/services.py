...
class PredictionService:
    ...
    def predict_from_dataframe(
        self, df: pl.DataFrame, config: PredictionConfig
        ...
        # Get predictions
        if_scores, dbscan_noise, combined_scores = (
            self.model_service.ensemble.predict_scores(X_scaled)
        )
        alerts, threshold = self.model_service.ensemble.predict_alerts(
            X_scaled, config.threshold_percentile
        )
    ...
    def predict_from_file(
        self, file_content: bytes, filename: str, config: PredictionConfig
    ) -> Tuple[List[PredictionResult], Dict[str, Any]]:
        """
    ...
    def predict_from_records(
        self, records: List[TransactionRecord], config: PredictionConfig
    ) -> Tuple[List[PredictionResult], Dict[str, Any]]:
        """
