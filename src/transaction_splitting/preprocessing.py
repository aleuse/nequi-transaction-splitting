...
from .config import WINDOW_DURATION, MIN_TRANSACTIONS_IN_GROUP, NUMERICAL_FEATURES
from .utils import (
...
class TransactionPreprocessor:
        ...
        candidate_groups = (
            ...
            .agg(
                [
                    ...
                    .implode()
                    .alias("all_amounts_in_group"),
                    pl.col("transaction_date").implode().alias("all_dates_in_group"),
                ]
            )
            .filter(pl.col("transaction_count") >= MIN_TRANSACTIONS_IN_GROUP)
            .with_columns(
                [
                    ...
                    .map_elements(calculate_amount_entropy, return_dtype=pl.Float64)
                    .alias("amount_entropy"),
                    # Average time delta
                    pl.col("all_dates_in_group")
                    .map_elements(
                        calculate_avg_time_delta_minutes, return_dtype=pl.Float64
                    )
                    .alias("avg_time_delta_minutes"),
                    # Round amounts flag
                    pl.col("all_amounts_in_group")
                    .map_elements(has_round_amounts, return_dtype=pl.Boolean)
    ...
    def prepare_features_for_ml(
        ...
        if len(existing_features) != len(NUMERICAL_FEATURES):
            missing = [
                col for col in NUMERICAL_FEATURES if col not in existing_features
            ]
